...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, onClick }) => {
  ...
  return (
    ...
    >
      <div className={`relative aspect-[2/3] overflow-hidden ${service.brand === 'filmsnyarwanda' ? 'rounded-t-2xl' : 'rounded-2xl'} transition-all duration-500 shadow-xl group-hover:shadow-red-600/20 group-hover:-translate-y-2`}>
        <img 
        ...
        {/* Studio Pick Badge */}
        {isRecommended && (
          <div className={`absolute top-4 left-4 z-20 ${service.brand === 'filmsnyarwanda' ? 'bg-yellow-400 text-black' : 'bg-red-600 text-white'} text-[8px] font-black px-2 py-1 rounded shadow-lg flex items-center gap-1 uppercase tracking-widest animate-in fade-in slide-in-from-left-2 duration-1000`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
          <div className={`w-16 h-16 ${service.brand === 'filmsnyarwanda' ? 'bg-yellow-400 text-black' : 'bg-red-600 text-white'} rounded-full flex items-center justify-center shadow-2xl transform scale-75 group-hover:scale-100 transition-transform`}>
      ...
      {service.brand === 'filmsnyarwanda' ? (
        <div className="bg-yellow-400 text-black py-3 px-4 text-center font-black text-[10px] uppercase tracking-widest shadow-lg relative z-10">
          {service.name} ({Math.floor(Math.random() * 50) + 1})
        </div>
      ) : (
